...
<form action="{{ route('buku.search') }}" method="GET" class="card mb-4">
    <div class="card-body">
        <div class="row g-3">
            <div class="col-md-4">
            ...
            <div class="col-md-3">
                ...
                <select name="kategori" class="form-select">
                    ...
                    <option value="{{ $item }}"
                        {{ (request('kategori') ?? ($kategori ?? '')) == $item ? 'selected' : '' }}>
                        {{ $item }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-3">
                ...
                <select name="ketersediaan" class="form-select">
                    <option value="semua" {{ (request('ketersediaan') ?? ($ketersediaan ?? 'semua')) == 'semua' ? 'selected' : '' }}>
                        Semua
                    </option>
                    <option value="tersedia" {{ (request('ketersediaan') ?? ($ketersediaan ?? '')) == 'tersedia' ? 'selected' : '' }}>
                        Tersedia
                    </option>
                    <option value="habis" {{ (request('ketersediaan') ?? ($ketersediaan ?? '')) == 'habis' ? 'selected' : '' }}>
                        Habis
                    </option>
                </select>
            </div>
...
<form action="{{ route('buku.bulk-delete') }}" method="POST"
    ...
    <thead>
        <tr>
            ...
            <th> Pilih Buku Yang Ingin Di Hapus</th>
        </tr>
    </thead>
